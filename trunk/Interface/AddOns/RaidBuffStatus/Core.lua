...
RBS_svnrev = {}
RBS_svnrev["Core.lua"] = select(3,string.find("$Revision: 416 $", ".* (.*) .*"))
...
function RaidBuffStatus:Tooltip(self, title, list, tlist, blist, slist, pallyblessingsmessagelist, itemcountlist, unknownlist, kingsbuffer, mightbuffer, gotitlist, zonelist)
...
function RaidBuffStatus:UpdateSpec(rcn, spec, c1, c2, c3)
	if spec == nil then
		...
		local t = 0
		if c1 > 30 then t = 1 end
		if c2 > 30 then t = 2 end
		if c3 > 30 then t = 3 end
		if t ~= 0 then
