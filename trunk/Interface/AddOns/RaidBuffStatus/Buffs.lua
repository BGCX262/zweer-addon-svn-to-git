...
RBS_svnrev["Buffs.lua"] = select(3,string.find("$Revision: 417 $", ".* (.*) .*"))

-- temporary HACK to conservatively handle cases where we cannot retrieve group talents
-- to be disabled when the server and libgrouptalents return reliable results
RaidBuffStatus.grouptalentsbroken = true
...
--					RaidBuffStatus:Debug("having to call notifyinspect for:" .. unit.unitid)
 					-- ticket 277: disabled until blizzard fixes inspect throttling
...
--								RaidBuffStatus:Debug(name .. " has " .. v .. " from " .. caster)
		...
		main = function(self, name, class, unit, raid, report)
			...
			if unit.hasbuff[BS[52127]] then -- Water Shield
				...
				if GT:GUIDHasTalent(raid.classes.SHAMAN[name].guid, BS[51525]) or   -- Static Shock talent
				   GT:GUIDHasTalent(raid.classes.SHAMAN[name].guid, BS[88766]) or -- Fulmination talent
				   RaidBuffStatus.grouptalentsbroken  then
	...
	missingblessing = {
		...
		click = function(self, button, down)
			local spell
			if report.mightmissing then
				spell = BS[19740]  -- Blessing of Might
			elseif report.kingsmissing then
		...
		raidbuff = true,
		whispertobuff = function(reportl, prefix)
			local b = RaidBuffStatus.BF.missingblessing:buffers()
...
--					end
	...
	drumskings = {
		...
		pre = function(self, raid, report)
			if not RaidBuffStatus:UseDrumsKings(raid) or not oRA or raid.isbattle then
				return
			end
