...
BRGroup.obj = LibStub:GetLibrary("LibDataBroker-1.1"):NewDataObject("Broker Group", {type = "data source", label = "Broker Group", icon = "Interface\\Buttons\\UI-GroupLoot-Dice-Up", text = "|cff9d9d9d" .. L["Solo"].."|r"})
...
local options = {
	...
	confdesc = {
		...
		setdungeondiff = {
			...
			get = function() return tostring(GetDungeonDifficulty()) end,
			set = function(_,v)
			...
			disabled = function()
				...
				if IsInInstance() or (inParty == 1 and isLeader == 0) or (UnitLevel("player") < 65 and GetDungeonDifficulty() == 1) then
					return true
				end
				return false
			end,
		...
		setraiddiff = {
			...
			get = function() return tostring(GetRaidDifficulty()) end,
			set = function(_,v)
				SetRaidDifficulty(tonumber(v))
			end,
			values = {
			...
			disabled = function()
				...
				if IsInInstance() or (inParty == 1 and isLeader == 0) or (UnitLevel("player") < 65 and GetRaidDifficulty() == 1) then
					return true
				end
				return false
			end,
...
function BRGroup.obj.OnTooltipShow(tooltip)
	tooltip:AddLine(_G["HIGHLIGHT_FONT_COLOR_CODE"].."Broker Group")
