...
ex.options[#ex.options + 1] = { var = "showFeatsId", default = false, label = "Show Achievement IDs", tip = "Advanced option: Show the ID of the achievement and the category it belongs to" };
...
function mod:OnAchievementReady()
	mod:HasData(true);
...
--			btn.val:SetText(tostring(entry.value).."\n|cffffffff"..format("%.4d.%.2d.%.2d",(entry.year or 0)+2000,entry.month or 0,entry.day or 0));
...
local function FeatsDropDown_InitFunc(dropDown,list)
	...
	tbl.text = "|cff00c0ffFeats of Strength Query"; tbl.value = -3; tbl.stub = -3;
	-- Tracked Achievements
	local isPlayer = ex:ValidateUnit() and UnitIsUnit(ex.unit,"player");
	if (isPlayer) then
		local tbl = list[#list + 1];
		tbl.text = "|cff00c0ffTracked Achievements |cffc0c0c0("..GetNumTrackedAchievements()..")"; tbl.value = -4; tbl.stub = -4;
	end
	-- Recent Achievements
	local tbl = list[#list + 1];
	tbl.text = "|cff00c0ffRecent Achievements"; tbl.value = -5; tbl.stub = -5;
	-- Achievements
	local tbl = list[#list + 1];
