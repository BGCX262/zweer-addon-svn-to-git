...
function ST:RegisterChannel(spell, tickspeed, duration)
	spell = FW:SpellName(spell);
	Drain[spell] = {tickspeed,duration};
end
	...
	if total > 0 --[[or expire==0]] then -- dont add if duration isnt greater than 0
		...
		if found then -- found a spell i may need to remove - if it's an exact match keep using it
			if st[found][8] == spell and st[found][11] == guid and abs(st[found][1]-expire)<maxlag then 
				...
				st:remove(found);
				st:insert(expire+extratime,FW:CastTime(spell),total,targetname,Track[spell][3],Track[spell][4],Track[spell][6],spell,targettype,0,guid,0,id,0,1,stacks,0,FlagToAction(frame1,spell,Track[spell][4]),icon,expire,total,haste,0);
			end
		else
			if Track[spell][7] > 0 then
				stacks = 1;
...
local function ST_VisibleInTimer(t6,t13,t15,t18,t11) -- using this in multiple functions, make sure the args are all set
	if t15>0 and t18~=FILTER_HIDE then
		if t13<=0 or t13>=maxi then
			if t11 == "" then
				return frame1.s.UnknownTarget;
			else
				return frame1.s.NoTarget;
			end
		elseif t11==FW.target then
			return frame1.s.Target
		elseif t11==FW.focus then 
			return frame1.s.Focus
		else
			return frame1.s.Other;	
		end
	else
		return false;
	end
			...
			if st[i][14]<=HIDDEN then
				...
				elseif st[i][11] == guid then
					...
					if t6<=UNIQUE or t6 >=HEAL then -- only check the 'normal' buff/debuff types
						...
						if expire then
							local remaining = st[i][1] - t;
							if ST_SpecialRefresh[t8] and remaining > 0 --[[and st[i][22] ~= CA:HasteFactor(t8)]] then -- don't use when a timer has already expired
								local diff = (expire-st[i][1]);
								if diff+st[i][23] > maxlag then -- compensate for the correction offset in testing if the debuff was updated
									local tick = (Tick[t8] or 3)*st[i][22]; -- keeps old haste now
									local newend = math.floor( (expire-t)/tick )*tick + remaining%tick + t;
									if newend - expire > 0.1 then
										newend = newend - tick;
									end
									st[i][1] = newend;
									...
									FW:Debug("using: "..newend.." original: "..expire);
								end
							else
...
FW:SetMainCategory(FWL.SPELL_TIMER,FW.ICON.ST,3,"TIMER","Timer","Timer",tab_data);
	...
	FW:SetSubCategory(FWL.TIMER_UNITS,FW.ICON.UNITS,6)
		...
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.TIMER_SHOW_UKNOWN,	"",					"UnknownTarget");
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.SHOW_WITHOUT_UNIT,	"",					"NoTarget");
